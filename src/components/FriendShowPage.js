...
export const Friend = (props) => {
  ...
  const thisUser = props.users.find(
    (user) => user["id"] == props.match.params.id
  );
  const findRaces = props.userRaces.filter(
    (userRace) => userRace["user"] == props.match.params.id
  );
  const incompleteRaces = props.userRaces.filter(
    (userRace) =>
      userRace["user"] == props.match.params.id && !userRace["completed"]
  );
  const userPhoto = props.photos.find(
    (photo) => photo["user"] == props.match.params.id
  );
  const completedRaces = props.userRaces.filter(
    (userRace) =>
      userRace["user"] == props.match.params.id && userRace["completed"]
  );
  ...
  const showRaces = () => {
    return findRaces.map((race) => {
      if (race["race_name"] && !race["completed"]) {
        return (
          <p className="heading">
            {race["race_name"]}
            <button
  ...
  const showCompletedRaces = () => {
    if (completedRaces.length === 0) {
      ...
      return completedRaces.map((race) => {
        if (race["completed"]) {
          if (race["race_name"]) {
            return <p>{race["race_name"]}</p>;
          } else {
            return <p>{race["distance"]}</p>;
          }
        }
      });
  };
  ...
  const showFollowers = () => {
    if (thisUser) {
      const followers = props.friends.filter(
        (friend) => friend["following"] == thisUser["id"]
      );
  ...
  const showFollowing = () => {
    if (typeof thisUser !== "undefined") {
      const following = props.friends.filter(
        (friend) => friend["follower"] == thisUser["id"]
      );
  ...
  return (
    <>
      ...
      <Link to="/friends">
        <button className="back">{`<< back to friends`}</button>
      </Link>
      <div className="friend">
        <div className="friend-profile">
          <div className="friend-info">
            ...
            <div>
              <button onClick={handleClick} className="send-message">
                Send Message
              </button>
            </div>
            {thisUser && incompleteRaces.length > 0 ? (
              <h2>{thisUser["first_name"]} is training for:</h2>
            ) : null}
            {thisUser && incompleteRaces.length === 0 ? (
              <h2>{thisUser["first_name"]} is not training for anything</h2>
            ) : null}
            {showRaces()}
          </div>
