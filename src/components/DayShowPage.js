...
export const DayShowPage = (props) => {
    ...
    const thisWorkout = props.activities.filter(activity =>
        activity["id"] === parseInt(props.match.params.id)
    )
    const userWorkouts = props.workouts.filter(workout =>
        workout["user"] === parseInt(localStorage.user)
    )

    const thisRace = () => {
        if (thisWorkout) {
            const thisRace = props.userRaces.filter(userRace =>
    ...
    const showWorkoutOrForm = () => {
        if (thisRace().length > 0) {
            if (userWorkouts.length > 0) {
                if (thisRace()) {
                    const userWorkout = userWorkouts.filter(workout =>
                    ...
                    if (userWorkout.length > 0) {
                        return [
                            ...
                            <h1>
                                Workout Type: {userWorkout[0]["workout_type"]}
                            </h1>,
                            userWorkout[0]["distance"] ?
                                <h1>
                                    Distance: {userWorkout[0]["distance"]}
                                </h1> 
                            : null,
                            userWorkout[0]["pace"] ?
                                <h1>
                                    Average pace per mile: {userWorkout[0]["pace"]}
                                </h1> 
                            : null,
                            <h1>
                                Duration: {userWorkout[0]["duration"]}
                            </h1>,
                            <h1>
                                Location: {userWorkout[0]["location"]}
                            </h1>
                        ]
                    }else {
                        return [
            ...
            {localStorage.token ?
                <>
                    ...
                    <div className="dayshow">
                        ...
                        {thisWorkout[0] &&
                            thisWorkout[0]["workout_type"] !== "rest" ?
