...
export const Friends = (props) => {
  ...
  const [alreadyFollowMessage, setAlreadyFollowMessage] = useState(
    "You are already following this runner"
  );

  const handleClick = (event) => {
    if (event.target.className === "follow-button") {
      const findFollowing = props.friends.find(
        (friend) =>
  ...
  const showFollowing = () => {
    return props.friends.map((friend) => {
      if (friend["follower"] === parseInt(localStorage.user)) {
        const findFriend = props.users.find(
          (user) => user["id"] === friend["following"]
        );
        if (typeof findFriend !== "undefined") {
          const userPhoto = props.photos.find(
            (photo) => photo["user"] === findFriend["id"]
          );
          if (userPhoto) {
            return (
  ...
  const sameRace = () => {
    const yourRaces = props.userRaces.filter(
      (userRace) =>
  ...
  const showSameRaceRunners = (name) => {
    const otherUsers = props.users.filter(
      (user) => user["id"] !== parseInt(localStorage.user)
    );
    return props.userRaces.map((userRace) => {
      if (userRace["race_name"] === name && !userRace["completed"]) {
        const sameRaceUser = otherUsers.find(
          (user) => user["id"] === userRace["user"]
        );
        if (sameRaceUser) {
          const userPhoto = props.photos.find(
            (photo) => photo["user"] === sameRaceUser["id"]
          );
          if (userPhoto) {
            return (
              <div className="friend-icon">
                <Link to={`/friends/${sameRaceUser["id"]}`}>
                  <div
  ...
  return (
    <>
      ...
      <div className="same-race-div">
        ...
        <div>{sameRace()}</div>
        <div className="same-race-runners">
          {viewUsers ? showSameRaceRunners(raceName) : null}
        </div>
      </div>
    </>
  );
