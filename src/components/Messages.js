...
class Messages extends Component {
  ...
  showInbox = () => {
    if (this.props.incomingMessages.length > 0) {
      return this.props.incomingMessages
        .sort(this.sortDate)
        .map((incomingMessage) => {
          if (incomingMessage["read"]) {
            const messageSender = this.props.users.find(
              (user) => user["id"] == incomingMessage["sender"]
            );
            if (messageSender) {
              return (
  ...
  showOutbox = () => {
    if (this.props.outgoingMessages.length > 0) {
      return this.props.outgoingMessages
        .sort(this.sortDate)
        .map((outgoingMessage) => {
          const messageRecipient = this.props.users.find(
            (user) => user["id"] == outgoingMessage["receiver"]
          );
          if (messageRecipient) {
            return (
              <div className="message-div">
                <p className="unread-message">
                  {messageRecipient["first_name"]}{" "}
                  {messageRecipient["last_name"]}
                </p>
